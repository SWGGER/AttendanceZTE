...
public class ExcelOper {
            ...
            for (int in = 0; in < wb.getNumberOfSheets(); in++) {
                ...
                for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    for (int index = 0; index < row.getLastCellNum(); index++) {
                        ...
                        line.put(getCellValue(sheet.getRow(0).getCell(index)).toString(), value);
                    }
    ...
    public static Map<String, Object> findsheetnameanddata(File f) throws Exception{

        Map<String, Object> remap = new HashMap<>();
        List<Map<String, Object>> datas = new ArrayList<Map<String, Object>>();
        Workbook wb = null;//Excel文档对象
        Sheet sheet = null;//工作表
        // 选择2003 .xls /2007 .xlsx ，构造Excel文档对象
        ...
        for(int m = 0 ; m < wb.getNumberOfSheets() ;m++){
            ...
            //获取Excel中的工作表 getSheet(String name)根据sheet名称来获取
            sheet = wb.getSheetAt(m);//根据下标获取sheet  getnumberofsheet 获取当前excel的sheet数
            //循环取行对象
            for(int i=0;i<=sheet.getLastRowNum();i++){	//遍历包括空行的所有行数
                //每行当做一个Map来存储
                ...
                //循环取列对象
                for(int index=0;index<row.getLastCellNum();index++){	//遍历所有列数
                    //获取列对象
                    Cell cell = row.getCell(index);
                    //获取列中数据
                    Object value = getCellValue(cell);
                    //key :  cell1  cell2  cell3 ...
                    data.put("col"+(index+1), value);
                }
                //存储行getCellValue(Cell cell)		LIST 中存所有 MAP
                datas.add(data);
            }
            remap.put(wb.getSheetName(m),datas);
        }
        return remap;
    }
